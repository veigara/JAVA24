...
public class Client {
	...
	public static void getFilmByCinema() {
		...
		Cinema[] cinemaByFilm = m.getCinemaByFilm(film);//根据电影院获取正在上映该电影的电影院的信息
		System.out.print("电影院名称：");
		for (int i = 0; i < cinemaByFilm.length; i++) {
			Cinema cinema = cinemaByFilm[i];
		...
		for (int i = 0; i < cinemaByFilm.length; i++) {
			if (cinemaByFilm[i] == null) break;
			Cinema cinema = cinemaByFilm[i];
			FilmList[] list = cinema.getList();
			for (int j = 0; j < list.length; j++) {
				if(list[j] == null) break;
				if (cinema.getName().equals(cinemaName)) {
					FilmList filmList = list[j];
					if (filmList.getFilm() == film) {
						FilmItem[] FilmItems = filmList.getList();
						for (int k = 0; k < FilmItems.length; k++) {
							if (FilmItems[k] == null) break;
							FilmItem item = FilmItems[k];
